...
function bubur(date,month,year,mauApa){
	...
	var viewsJune=[5052,5064,5072,5082,5089,5101,5109,5117,5125,5134,5143,5154,5161,5172,5178,5185,5199,5206,5211,5220,5235,5242,5250,5263,5273,5281,5290,5298,5306,5316];
	var viewsJuly=[5326,5333,5341,5346,5355,5367,5375,5389,5396,5405,5415,5424,5431,5440,5449,5457,5465,5472,5479,5488,5496,5505,5511,5524,5535];
	var viewsAugust=[];
	if (year==2021){
...
function bawal(date,month,year,mauApa){
	...
	var viewsJune=[5294,5313,5339,5362,5390,5417,5434,5452,5477,5491,5511,5532,5546,5571,5594,5616,5646,5667,5682,5701,5728,5753,5769,5792,5824,5842,5861,5873,5888,5899];
	var viewsJuly=[5905,5918,5927,5935,5947,5964,5978,5993,6005,6016,6027,6035,6047,6059,6074,6085,6095,6105,6114,6123,6134,6144,6155,6173,6189];
	var viewsAugust=[];
	if (year==2021){
		if(month==0){return viewsMay.length;}
		else if(month==5&&mauApa==1){return viewsMay[date-1];}
...
function lidahBuaya(date,month,year,mauApa){
	...
	var viewsJune=[3359,3368,3377,3387,3394,3405,3414,3421,3429,3436,3441,3453,3465,3473,3479,3487,3494,3501,3507,3516,3525,3533,3541,3552,3563,3569,3579,3587,3598,3609];
	var viewsJuly=[3619,3626,3634,3637,3642,3652,3660,3673,3680,3692,3700,3711,3716,3725,3735,3744,3756,3762,3769,3777,3788,3793,3804,3816,3825];
	var viewsAugust=[];
	if (year==2021){
		if(month==0){return viewsMay.length;}
		else if(month==5&&mauApa==1){return viewsMay[date-1];}
...
function sopSopan(date,month,year,mauApa){
	...
	var viewsJune=[3554,3565,3579,3589,3605,3618,3626,3639,3649,3659,3668,3679,3692,3701,3712,3722,3731,3739,3745,3754,3764,3774,3780,3791,3803,3812,3823,3833,3843,3857];
	var viewsJuly=[3868,3876,3887,3894,3911,3929,3940,3953,3961,3984,3994,4008,4014,4023,4033,4043,4051,4061,4070,4079,4089,4096,4104,4122,4135];
	var viewsAugust=[];
	if (year==2021){
		if(month==0){return viewsMay.length;}
		else if(month==5&&mauApa==1){return viewsMay[date-1];}
...
function oyong(date,month,year,mauApa){
	...
	var viewsJune=[2966,2977,2988,3000,3017,3034,3047,3059,3074,3085,3095,3107,3119,3131,3138,3148,3167,3176,3187,3198,3214,3228,3238,3252,3265,3274,3291,3304,3319,3331];
	var viewsJuly=[3342,3351,3358,3364,3373,3384,3393,3408,3418,3432,3441,3454,3462,3471,3483,3493,3502,3514,3522,3529,3538,3544,3552,3563,3573];
	var viewsAugust=[];
	if (year==2021){
		if(month==0){return viewsMay.length;}
		else if(month==5&&mauApa==1){return viewsMay[date-1];}
...
function cacana(date,month,year,mauApa){
	...
	var viewsJune=[2443,2456,2466,2475,2483,2497,2508,2516,2525,2536,2542,2552,2562,2570,2577,2591,2604,2615,2624,2635,2648,2658,2666,2683,2695,2705,2715,2727,2736,2748];
	var viewsJuly=[2757,2766,2775,2780,2786,2800,2811,2821,2828,2841,2852,2863,2870,2879,2890,2898,2909,2919,2926,2933,2942,2946,2955,2967,2977];
	var viewsAugust = [];
	if (year==2021){
		if(month==0){return viewsMay.length;}
		else if(month==5&&mauApa==1){return viewsMay[date-1];}
...
function berkebun(date,month,year,mauApa){
	...
	var viewsJune=[1564,1581,1594,1607,1617,1632,1641,1654,1663,1675,1683,1692,1700,1708,1715,1729,1743,1754,1762,1774,1788,1796,1805,1821,1833,1845,1857,1870,1888,1902];
	var viewsJuly=[1913,1924,1934,1940,1952,1964,1975,1985,1994,2005,2014,2025,2039,2054,2068,2083,2098,2103,2113,2125,2135,2142,2152,2168,2179];
	var viewsAugust=[];
	if (year==2021){
		if(month==0){return viewsMay.length;}
		else if(month==5&&mauApa==1){return viewsMay[date-1];}
...
function wiladatika(date,month,year,mauApa){
	...
	var viewsJune=[627,644,662,679,693,711,725,741,754,767,779,791,808,824,835,849,867,886,903,930,960,1025,1061,1086,1113,1149,1170,1203,1242,1279];
	var viewsJuly=[1304,1332,1349,1360,1375,1400,1425,1464,1478,1498,1518,1540,1547,1564,1590,1605,1626,1643,1654,1666,1674,1683,1697,1716,1733];
	var viewsAugust=[];
	if (year==2021){
		if(month==0){return viewsMay.length;}
		else if(month==5&&mauApa==1){return viewsMay[date-1];}
...
function apel(date,month,year,mauApa){
	...
	var viewsJune=[223,241,283,309,333,376,425,476,541,582,639,682,738,798,838,908,1046,1126,1195,1343,1453,1578,1699,1841,1951,2070,2250,2418,2532,2615]
	var viewsJuly=[2695,2764,2840,2882,2952,3022,3104,3165,3231,3314,3392,3474,3582,3667,3741,3821,3913,4004,4072,4141,4199,4258,4313,4381,4447];
	var viewsAugust=[];
	if (year==2021){
		if(month==0){return viewsMay.length;}
		else if(month==5&&mauApa==1){return viewsMay[date-1];}
...
function jeruk(date,month,year,mauApa){
	...
	var viewsJuly=[130,187,222,243,270,310,373,443,484,528,572,610,647,676,709,731,766,793,822,895,967,1034,1124,1198,1264];
	var viewsAugust=[];
	if (year==2021){
		if(month==0){return viewsMay.length;}
		else if(month==6&&mauApa==1){return viewsJune[date-1];}
