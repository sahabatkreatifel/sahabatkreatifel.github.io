...
function cacana(date,month,year,mauApa){
	...
	var viewsJune = [2443,2456,2466,2475,2483,2497,2508,2516,2525,2536,2542,2552,2562,2570,2577,2591,2604,2615,2624,2635,2648,2658,2666,2683,2695,2705,2715,2727];				
	var viewsJuly = [];
	var viewsAugust = [];
	if (year == 2021){
		if (month == 0){return viewsMay.length;}
...
function wiladatika(date,month,year,mauApa){
	...
	var viewsJune = [627,644,662,679,693,711,725,741,754,767,779,791,808,824,835,849,867,886,903,930,960,1025,1061,1086,1113,1149,1170,1203];
	var viewsJuly = [];
	var viewsAugust = [];
	if (year == 2021){
		if (month == 0){return viewsMay.length;}
